...
public class EnderStorageRecipe implements IRecipe {
    ...
    public boolean matches(InventoryCrafting ic, World var2) {
        for (int row = 0; row < 2; row++) if (offsetMatchesDyes(ic, 0, row)) return true;
    ...
    private boolean offsetMatchesDyes(InventoryCrafting ic, int col, int row) {
        if (!stackMatches(
                ic.getStackInRowAndColumn(col + 1, row + 1),
                Item.getItemFromBlock(EnderStorage.blockEnderChest)))
            return false;
    ...
    public ItemStack getCraftingResult(InventoryCrafting ic) {
        for (int row = 0; row < 2; row++) {
            if (!offsetMatchesDyes(ic, 0, row)) continue;
            ...
            result.setItemDamage(
                    EnderStorageManager.getFreqFromColours(colour3, colour2, colour1)
                            | freqowner.getItemDamage() & 0xF000);
            return result;
        }
        return null;
    }
    ...
    private int recolour(int i, int row, int freq, InventoryCrafting ic) {
        ...
        if (dyeType >= 0) return ~dyeType & 0xF;
    ...
    public static void removeVanillaChest() {
        ...
        while (iterator.hasNext()) {
            ...
            if (r != null && r.getItem() == Item.getItemFromBlock(Blocks.ender_chest)) iterator.remove();
        }

        if (!EnderStorage.removeVanillaRecipe) CraftingManager.getInstance().addRecipe(
    ...
    private static void addNormalRecipies() {
        for (int i = 0; i < 16; i++) {
            GameRegistry.addRecipe(
                    new ItemStack(EnderStorage.blockEnderChest, 1, EnderStorageManager.getFreqFromColours(i, i, i)),
                    "bWb",
                    "OCO",
                    "bpb",
                    'b',
            ...
            GameRegistry.addRecipe(
                    new ItemStack(EnderStorage.itemEnderPouch, 1, EnderStorageManager.getFreqFromColours(i, i, i)),
                    "blb",
                    "lpl",
                    "bWb",
                    'b',
            ...
            GameRegistry.addRecipe(
                    new ItemStack(
                            EnderStorage.blockEnderChest,
                            1,
                            1 << 12 | EnderStorageManager.getFreqFromColours(i, i, i)),
                    "bWb",
                    "OCO",
                    "bpb",
                    'b',
    ...
    public static int getDyeType(ItemStack item) {
        ...
        if (item.getItem() == Items.dye) return item.getItemDamage();
        for (int i = 0; i < 16; i++) {
    ...
    public static String[] oreDictionaryNames = new String[] { "dyeBlack", "dyeRed", "dyeGreen", "dyeBrown", "dyeBlue",
    ...
    public static int getDyeColour(String string) {
        for (int i = 0; i < 16; i++) if (oreDictionaryNames[i].substring(3).equalsIgnoreCase(string)) return i;
        return -1;
    }
}
