...
public class TankSynchroniser {
    ...
    public static abstract class TankState {
        ...
        public void update(boolean client) {
            ...
            if (client) {
                ...
                else c_liquid = FluidUtils.copy(s_liquid, 0);
                ...
                if (!s_liquid.isFluidEqual(c_liquid)) {
                    sendSyncPacket();
                    c_liquid = s_liquid;
                } else if (Math.abs(c_liquid.amount - s_liquid.amount) > 250
                        || (s_liquid.amount == 0 && c_liquid.amount > 0)) {
                            sendSyncPacket();
                            c_liquid = s_liquid;
                        }
            ...
            if ((b_liquid.amount == 0) != (a_liquid.amount == 0) || !b_liquid.isFluidEqual(a_liquid)) onLiquidChanged();
        }
        ...
        public void sync(FluidStack liquid) {
            s_liquid = liquid;
            if (!s_liquid.isFluidEqual(c_liquid)) f_liquid = c_liquid.copy();
        }
    }
    ...
    public static class PlayerItemTankState extends TankState {
        ...
        @Override
        public void update(boolean client) {
            if (tracking || client) super.update(client);
        }
    }
    ...
    public static class PlayerItemTankCache {
            ...
            if (state == null) {
                ...
                tankStates.put(
                        ...
                        state = new PlayerItemTankState(
                                player,
                                (EnderLiquidStorage) EnderStorageManager.instance(false)
        ...
        public void sync(int freq, String owner, FluidStack liquid) {
            String key = key(freq, owner);
            PlayerItemTankState state = tankStates.get(key);
            if (state == null) tankStates.put(key, state = new PlayerItemTankState());
            state.sync(liquid);
        }

        public void update() {
            for (Entry<String, PlayerItemTankState> entry : tankStates.entrySet()) entry.getValue().update(client);
        ...
        public void handleVisiblityPacket(PacketCustom packet) {
            ...
            for (int i = 0; i < k; i++) track(packet.readUShort(), packet.readString(), true);
            k = packet.readUShort();
            for (int i = 0; i < k; i++) track(packet.readUShort(), packet.readString(), false);
        }
    }
    ...
    public void onPlayerLogin(PlayerLoggedInEvent event) {
        playerItemTankStates
                .put(event.player.getCommandSenderName(), new PlayerItemTankCache((EntityPlayerMP) event.player));
    }
