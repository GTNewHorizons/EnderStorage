...
public class ItemEnderPouch extends Item {
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (tile instanceof TileEnderChest && player.isSneaking()) {
            ...
            else stack.getTagCompound().setString("owner", "global");
    ...
    public int getIconIndex(ItemStack stack, int renderPass) {
        if (renderPass == 0) {
            ...
            if (((EnderItemStorage) EnderStorageManager.instance(true)
                    .getStorage(getOwner(stack), stack.getItemDamage() & 0xFFF, "item")).openCount() > 0)
                i |= 1;
            if (!getOwner(stack).equals("global")) i |= 2;
            return i;
        }
    ...
    public void registerIcons(IIconRegister register) {
        ...
        for (int i = 16; i < 64; i++) spriteSheet.requestIndicies(i);
