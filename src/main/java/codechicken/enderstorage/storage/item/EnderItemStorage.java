...
public class EnderItemStorage extends AbstractEnderStorage implements IInventory {
    ...
    private void alignSize() {
        if (configSize > size) {
            ...
            int numStacks = 0;
            for (ItemStack item : items) if (item != null) numStacks++;
    ...
    public NBTTagCompound saveToTag() {
        if (size != configSize && open == 0) alignSize();
    ...
    public void openInventory() {
        ...
        synchronized (this) {
            open++;
            if (open == 1) EnderStorageSPH.sendOpenUpdateTo(null, owner, freq, true);
        }
    }
    ...
    public void closeInventory() {
        ...
        synchronized (this) {
            open--;
            if (open == 0) EnderStorageSPH.sendOpenUpdateTo(null, owner, freq, false);
        }
    }
    ...
    public void openSMPGui(EntityPlayer player, final String name) {
        ServerUtils.openSMPContainer(
                ...
                new ContainerEnderItemStorage(player.inventory, this, false),
