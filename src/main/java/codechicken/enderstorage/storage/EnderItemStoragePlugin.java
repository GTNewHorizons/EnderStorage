...
public class EnderItemStoragePlugin implements EnderStoragePlugin {
    ...
    public void loadConfig(ConfigTag config) {
        configSize = config.getTag("storage-size")
                .setComment("The size of each inventory of EnderStorage. 0 = 3x3, 1 = 3x9, 2 = 6x9").getIntValue(1);
        if (configSize < 0 || configSize > 2) configSize = 1;
    }
